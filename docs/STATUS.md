...
- [x] **FP6** (front) docente: asignación masiva (buscador + checkboxes) + evaluar con un clic — ver `docs/reports/FP6-report.md`
...
- **R23** Escenario sin JSON: `datos_esperados` pasa a nullable/opcional; el docente escribe el caso en lenguaje natural (título + descripción + evento). La evaluación automatizada queda oculta en la UI (backend `evaluar` sigue pero sin uso). (Coste: se pierde la corrección automática por ahora; se reintroduce luego con UI intuitiva, no JSON.)
- **R24** Navegación por rol minimal: estudiante solo ve "Mis escenarios"; docente solo "Docente". Las fichas (Datos Básicos/Complementarios) se acceden SOLO desde el flujo del escenario. Caracterización queda fuera del flujo educativo (es institucional, no parte de un caso). (Coste: no hay acceso suelto a las fichas; se llega siempre vía escenario.)
- **R25** Flujo del escenario encadenado: diligenciar → Datos Básicos → Datos Complementarios → entregar (marca COMPLETADO). La entrega ocurre al final (tras complementaria). (Coste: la complementaria sigue siendo específica de Ofídico; otros eventos no adaptan el form.)
- **R26** Complementaria GENÉRICA: `contenido` es un `dict` libre (se eliminó la validación ofídico); el form envía `{ fecha_evento, lugar, descripcion }`. La validación por evento se reintroduce cuando haya esquemas por evento. RBAC de complementarias desacoplado (cualquier rol autenticado). (Coste: sin validación estructurada del contenido por ahora.)
- **R27** Diseño: se adopta el lenguaje de los mockups de `docs/base/` (secciones `.card` con badge numerado, grid 3 col, chips Sí/No) vía componente `Section`. "Ver ficha" del docente agrupado en secciones + muestra la complementaria (render genérico clave→valor). (Coste: los forms heredan el lenguaje visual; queda pulir datos-basicos/caracterizacion más a fondo si se quiere.)
- **R28** Área docente en páginas separadas: `/docente/escenarios` (lista), `/docente/escenarios/nuevo` (crear), `/docente/escenarios/[id]` (detalle con asignaciones de ESE escenario). La experiencia del docente con ~100 estudiantes es prioritaria: asignaciones dentro del caso, no una lista global. (Coste: sin endpoint `GET /docente/escenarios/{id}` individual; el detalle filtra la lista. Se puede añadir luego.)
...
- `2026-09-25` **FP6 completo → FLUJO ESTUDIANTE+DOCENTE TERMINADO (6/6 fases)**. Frontend: lint + build limpios. Docente: asignación masiva con buscador + checkboxes (muestra identificación), "Ver ficha" resumen, "Evaluar" con un clic (usa escenario_id + ficha_basica_id de la asignación, sin IDs a mano). Backend final: pytest `77 passed`, ruff clean.
- `2026-09-25` **FP7 (iteración) — simplificación del flujo**. Backend: `datos_esperados` nullable (migración). Frontend: nav por rol (solo "Mis escenarios"/"Docente"); creador de escenario sin JSON (título + descripción natural + select de evento desde `/catalogos/eventos`); evaluación oculta; flujo encadenado básica → complementaria → entregar. Backend `77 passed`, frontend lint + build limpios. Deferred: la complementaria sigue específica de Ofídico (otros eventos no adaptan).
...
- Datos complementarios siguen standalone (no vinculados al escenario en la entrega; la evaluación de `contenido` tolera ausencia).
- `clasificacion_caso`/`condicion_final` se muestran como valor crudo (sin etiquetas "Sospechoso/Probable/...").
- `AsignacionOut` no incluye `numero_identificacion` (se resuelve desde `GET /docente/estudiantes` en cliente).
- Warning de deprecación `HTTP_422_UNPROCESSABLE_ENTITY` en FastAPI 0.141 (123 warnings, cosmético).
## Descubrimientos
